    ...
    def rec_dynamic(self, dt:ti.f32, frame:ti.i32, wind_v: ti.f32):
        ...
        for i in self.rec_pos_f:
            for j in range(200):
                ...
                t = dt * frame
                self.rec_pos_f[i][0] = self.rec_pos_f[i][0] + self.PM[i] * ti.sin(w ** 2.0 / 9.8 * self.rec_pos_f[i][0] + w * t)
                self.rec_pos_f[i][1] = self.rec_pos_f[i][1] + self.PM[i] * ti.cos(w ** 2.0 / 9.8 * self.rec_pos_f[i][0] + w * t)
